<template>
  <section class="wh_content" @touchmove="fn">
    ...
    <div v-if="showIndicator" class="wh_indicator">
      <div v-for="(tag,$index) in slidesLength" :key="$index" :class="{ wh_show_bgcolor: index-1==$index }" class="wh_indicator_item" />
    </div>

    <div v-if="slidesLength > 1 && index < slidesLength" class="wh_next" :style="{backgroundImage: `url(${NextIconUrl})`}" @click="nextSlide" />

    <div v-if="slidesLength > 1 && index > 1" class="wh_prev" :style="{backgroundImage: `url(${PrevIconUrl})`}" @click="prevSlide" />
  </section>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      index: 1,
      NextIconUrl: NextIcon,
      PrevIconUrl: PrevIcon
    }
  },
  mounted() {
