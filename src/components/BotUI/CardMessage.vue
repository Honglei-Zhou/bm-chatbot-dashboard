<template>
  <div class="sc-message--card" :style="messageColors">
    <Swiper v-if="data.elements.length>0" ref="swiper" :auto-play="false" :show-indicator="false">
      <Slide v-for="(item,index) in data.elements" :key="index">
        ...
        <div class="sc-message--card-title" :style="messageColors">
          {{ item.title }}
        </div>
        <div v-if="item.subtitle" class="sc-message--card-text" :style="messageColors">{{ item.subtitle }}</div>
        <div v-if="item.text" class="sc-message--card-text" :style="messageColors">{{ item.text }}<p v-if="item.meta" class="sc-message--meta" :style="messageColors">{{ item.meta }}</p></div>
        <div v-if="item.buttons && item.buttons.length > 0">
          <button
            ...
            :style="messageColors"
            @click="$emit('sendItem', cardButton)"
          >{{ cardButton.title }}</button>
        </div>
      </Slide>
    </Swiper>
  </div>
